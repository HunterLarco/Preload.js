			...
			this.open = (function PreloadOpener(url,type){
				...
				this.read = (function PreloadReader(){
					...
					xhr.addEventListener("progress",(function(event){
						if (event.lengthComputable){
							var percentLoaded = event.loaded / event.total;
							try{this.onprogress(PreloadProgressEvent(percentLoaded*0.95,event.loaded,event.totalSize));}catch(e){};
						}else{
							try{this.onprogress(PreloadProgressEvent(undefined,event.loaded,event.totalSize));}catch(e){};
						};
					}).bind(system),false);
